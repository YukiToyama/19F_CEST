...
fq1,data1,error1 = np.loadtxt("5Hz.out").transpose()
fq2,data2,error2 = np.loadtxt("14Hz.out").transpose()
...
# rate constants
fit_params.add('kAB',value=5,vary=True)
fit_params.add('kBA',value=15,vary=True)

# Relaxation
fit_params.add('R2A',value=20)
fit_params.add('R2B',value=20)
fit_params.add('R1',value=2.7548,vary=False) # Fixed to the known value
fit_params.add('sigma',value=0,vary=True)
...
def objective(fit_params):
    ...
    calc1 = cest_profile.calc_cr_eq_T0(fq1,Trelax_1,b1_1,b1_1*inhom,CS_A,CS_B,R2A,R2B,R1,sigma,kAB,kBA)
    residual = np.append(residual, (data1[1:]/data1[0] - calc1[1:]/calc1[0]))
    
    calc2 = cest_profile.calc_cr_eq_T0(fq2,Trelax_2,b1_2,b1_2*inhom,CS_A,CS_B,R2A,R2B,R1,sigma,kAB,kBA)
...
sim1 = cest_profile.calc_cr_eq_T0(fq1,Trelax_1,b1_1,b1_1*inhom,
                                      opt_CS_A,opt_CS_B,opt_R2A,opt_R2B,
                                      opt_R1,opt_sigma,opt_kAB,opt_kBA)
sim2 = cest_profile.calc_cr_eq_T0(fq2,Trelax_2,b1_2,b1_2*inhom,
                                      opt_CS_A,opt_CS_B,opt_R2A,opt_R2B,
                                      opt_R1,opt_sigma,opt_kAB,opt_kBA)
##################
# Update Parameters
...
fit_params.add('R1',value=opt_R1,vary=False)
fit_params.add('sigma',value=opt_sigma,vary=True)
