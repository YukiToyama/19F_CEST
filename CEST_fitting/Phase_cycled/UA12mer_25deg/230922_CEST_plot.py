...
o1p = -200 
sfo1 = 564.479
correction = +0.08 # temperature correction
...
bestfit_1 = cest_profile.calc_cr_pc(fq1,Trelax_1,b1_1,b1_1*inhom,CS_A,CS_B,R2A,R2B,R1,sigma,kAB,kBA)
bestfit_2 = cest_profile.calc_cr_pc(fq2,Trelax_2,b1_2,b1_2*inhom,CS_A,CS_B,R2A,R2B,R1,sigma,kAB,kBA)

MC_curve_1 = np.zeros([len(MC_kAB),len(fq1)])
MC_curve_2 = np.zeros([len(MC_kAB),len(fq2)])

for i in range(len(MC_kAB)):
    MC_curve_1[i] = cest_profile.calc_cr_pc(fq1,Trelax_1,b1_1,b1_1*inhom,
                                            MC_CS_A[i],MC_CS_B[i],MC_R2A[i],
                                            MC_R2B[i],MC_R1[i],MC_sigma[i],MC_kAB[i],MC_kBA[i])
    MC_curve_2[i] = cest_profile.calc_cr_pc(fq2,Trelax_2,b1_2,b1_2*inhom,
                                            MC_CS_A[i],MC_CS_B[i],MC_R2A[i],
                                            MC_R2B[i],MC_R1[i],MC_sigma[i],MC_kAB[i],MC_kBA[i])
...
ax1.plot([CS_Appm,CS_Appm],[0,ymax],ls="--",color='dodgerblue',linewidth=0.5)
ax1.plot([CS_Bppm,CS_Bppm],[0,ymax],ls="--",color='turquoise',linewidth=0.5)
