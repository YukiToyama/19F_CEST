...
for i in range(len(tauC)):
    ...
    # Manually normalize the data by multiplying the whole profile by exp(-R1TEx) 
    normfac = np.exp(F_relax.rho_FH(tauC[i])*Trelax_1)
    ax1.plot(fqlist[1:],sim1[1:]/sim1[0]*normfac,c=color[i],ls=ls[i],linewidth=1,label="$\\tau_C$ = "+str(round(1E9*tauC[i],1))+" (ns)")
    ax2.plot(fqlist[1:],sim1[1:]/sim1[0]*normfac,c=color[i],ls=ls[i],linewidth=1,label="$\\tau_C$ = "+str(round(1E9*tauC[i],1))+" (ns)")
