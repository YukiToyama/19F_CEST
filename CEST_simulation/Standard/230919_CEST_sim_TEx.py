...
for k in range(len(sigma)):
    ...
    # Trelax = 0    
    ref = cest_profile.calc_cr_eq(fqlist,0,b1_1,b1_1*inhom,
                                          CS_A,CS_B,R2A,R2B,
                                          R1,sigma[k],kAB,kBA)
    for i in range(len(Trelax)):
        sim1 = cest_profile.calc_cr_eq(fqlist,Trelax[i],b1_1,b1_1*inhom,
                                              CS_A,CS_B,R2A,R2B,
                                              R1,sigma[k],kAB,kBA)
    
        ax1.plot(fqlist,sim1/ref,c=color[k],ls=ls[i],linewidth=1,label="$T_{Ex}$ = "+str(int(Trelax[i]*1000))+" (ms)")
