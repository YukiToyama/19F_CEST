...
import cest_matrix_HJ as cest_matrix_HJ
import F_relax as F_relax
importlib.reload(cest_matrix_HJ)
importlib.reload(F_relax)

##############
# Paramaeters
##############
outname = "J_DD_standard_normalized"
phasecycle = False
...
for j in range(len(tauC)):
    ...
    for k in range(len(b1_list)):
        ...
        for i in range(len(fqlist)):
            ...
            initial = cest_matrix_HJ.initial(kAB, kBA, Ieq, Seq)
            L = cest_matrix_HJ.L(offsetA, offsetB, w1Ix, w1Iy,J)

            R1I = F_relax.rho_FH(tauC[j])
            R1S = F_relax.rho_HF(tauC[j])+ F_relax.rho_HH(tauC[j])
            sigma_IS = F_relax.sigma(tauC[j])           
            
            Gamma = cest_matrix_HJ.Gamma(R1I, R1S, R2I, sigma_IS, kAB, kBA, Ieq, Seq)
            ...
            if phasecycle==True:
                initial2 = cest_matrix_HJ.initial2(kAB, kBA, Ieq, Seq)
    ...
    ax2.plot(fqlist[1:],magI[1:]/initial[3],label="$\\tau_C$ = "+str(round(tauC[j]*1e9,2))+" [ns]",color=color[j])
...
ax1.plot([lf,lf,hf,hf,lf],[lh,hh,hh,lh,lh],ls="--",color="black",linewidth=1)
...
ax2.set_title("Close-up of the dotted region")
